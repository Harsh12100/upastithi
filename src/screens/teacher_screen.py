        ...
        if st.button('Run Face Analysis', use_container_width=True, type='secondary', disabled=not has_photos):
            with st.spinner('Deep scanning classroom photos...'):
                ...
                for idx, img in enumerate(st.session_state.attendance_images):
                    ...
                    detected, all_students, face_count = predict_attendance(img_np)
                ...
                enrolled_res = supabase.table('subject_students').select(
                    "*, students!subject_students_student_id_fkey(*)"
                ...
                if not enrolled_students:
                    ...
                    for node in enrolled_students:
                        ...
                        sources = all_detected_ids.get(student['id'], [])
    ...
    if subjects:
        for sub in subjects:
            ...
            if sub.get('subject_students'):
                if isinstance(sub['subject_students'], list) and len(sub['subject_students']) > 0:
                    student_count = sub['subject_students'][0].get('count', 0)

            class_count = len(sub.get('attendance_logs', []))
            stats = [
                ("🫂", "Students", student_count),
                ("🕰️", "Classes", class_count),
            ]
